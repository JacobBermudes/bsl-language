...
- **Javadoc — про контракт, а не про вызовы.** Javadoc класса/метода описывает *что* он делает:
  ...
  вызывающей стороны, тестах или (для обзора подсистемы) в `package-info`/`docs`. Исключение —
  если без сценария контракт реально непонятен; согласовывай с пользователем.
